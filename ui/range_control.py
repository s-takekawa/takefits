from PyQt6.QtWidgets import QWidget, QGridLayout, QLineEdit, QPushButton, QLabel, QMessageBox, QHBoxLayout
...
class RangeControlPanel(QWidget):
    ...
    def initUI(self):
        ...
        if self.fits_viewer.data.ndim > 2:
            ...
            ymin_val = zy_limits.get('y_min', str(self.original_ylim[0]))
            ymax_val = zy_limits.get('y_max', str(self.original_ylim[1]))
        ...
        if len(self.subwindows) > 1 and self.subwindows[0]:
    ...
    def _get_range_file_path(self):
        configured = self.fits_viewer.config_manager.config.get('range_file', 'takefits.range')
    ...
    def _sync_from_common(self, plane=None):
        updated = False

        def copy_xy():
            if all(getattr(Common, attr, None) for attr in ("xmin_input_xy", "xmax_input_xy", "ymin_input_xy", "ymax_input_xy")):
                self.x_min_input.setText(Common.xmin_input_xy.text())
                self.x_max_input.setText(Common.xmax_input_xy.text())
                self.y_min_input.setText(Common.ymin_input_xy.text())
                self.y_max_input.setText(Common.ymax_input_xy.text())
                return True
            return False

        def copy_xz():
            if all(getattr(Common, attr, None) for attr in ("xmin_input_xz", "xmax_input_xz", "zmin_input_xz", "zmax_input_xz")):
                self.x_min_input.setText(Common.xmin_input_xz.text())
                self.x_max_input.setText(Common.xmax_input_xz.text())
                self.z_min_input.setText(Common.zmin_input_xz.text())
                self.z_max_input.setText(Common.zmax_input_xz.text())
                return True
            return False

        def copy_zy():
            if all(getattr(Common, attr, None) for attr in ("zmin_input_zy", "zmax_input_zy", "ymin_input_zy", "ymax_input_zy")):
                self.z_min_input.setText(Common.zmin_input_zy.text())
                self.z_max_input.setText(Common.zmax_input_zy.text())
                self.y_min_input.setText(Common.ymin_input_zy.text())
                self.y_max_input.setText(Common.ymax_input_zy.text())
                return True
            return False

        if plane in (None, 'xy'):
            updated = copy_xy() or updated

        if self.fits_viewer.data.ndim > 2:
            if plane in (None, 'xz') and hasattr(self, 'z_min_input'):
                updated = copy_xz() or updated
            if plane in (None, 'zy') and hasattr(self, 'z_min_input'):
                updated = copy_zy() or updated
    ...
    def set_x_range(self):
        ...
        try:
            ...
            Common.xmin_input_xy.setText(str(x_min))
            Common.xmax_input_xy.setText(str(x_max))
            ...
            if len(self.subwindows) > 0 and self.subwindows[0]:  # SubWindow1 (XZ plane)
                ...
                Common.xmin_input_xz.setText(str(x_min))
                Common.xmax_input_xz.setText(str(x_max))

        except ValueError:
            QMessageBox.warning(self, 'Invalid Input', 'Please enter valid numeric values for the X range.')
            return

        self._sync_from_common('xy')
    def set_y_range(self):
        ...
        try:
            ...
            Common.ymin_input_xy.setText(str(y_min))
            Common.ymax_input_xy.setText(str(y_max))
            ...
            if len(self.subwindows) > 0 and self.subwindows[1]:  # SubWindow1 (XZ plane)
                self.subwindows[1].ax.set_ylim(yp_min, yp_max)
                self.subwindows[1].canvas.draw_idle()
                Common.ymin_input_zy.setText(str(y_min))
                Common.ymax_input_zy.setText(str(y_max))
    ...
    def set_z_range(self):
        ...
        try:
            ...
            if len(self.subwindows) > 0 and self.subwindows[0]:  # SubWindow1 (XZ plane vertical axis)
                ...
                self.subwindows[0].canvas.draw_idle()
                Common.zmin_input_xz.setText(str(z_min))
                Common.zmax_input_xz.setText(str(z_max))
            ...
            if len(self.subwindows) > 1 and self.subwindows[1]:  # SubWindow2 (ZY plane horizontal axis)
                ...
                self.subwindows[1].canvas.draw_idle()
                Common.zmin_input_zy.setText(str(z_min))
                Common.zmax_input_zy.setText(str(z_max))
        ...
        self._sync_from_common('xz')
        self._sync_from_common('zy')
    def update_ranges(self, plane, xlim, ylim):
        ...
        if plane == "xy" and Common.xmin_input_xy is not None:
            self.x_min_input.setText(Common.xmin_input_xy.text())
            self.x_max_input.setText(Common.xmax_input_xy.text())
            self.y_min_input.setText(Common.ymin_input_xy.text())
            self.y_max_input.setText(Common.ymax_input_xy.text())
        ...
        if plane == "xz" and self.fits_viewer.data.ndim > 2 and Common.xmin_input_xz is not None:
            self.x_min_input.setText(Common.xmin_input_xz.text())
            self.x_max_input.setText(Common.xmax_input_xz.text())
            self.z_min_input.setText(Common.zmin_input_xz.text())
            self.z_max_input.setText(Common.zmax_input_xz.text())
        ...
        if plane == "zy" and self.fits_viewer.data.ndim > 2 and Common.zmin_input_zy is not None:
            self.z_min_input.setText(Common.zmin_input_zy.text())
            self.z_max_input.setText(Common.zmax_input_zy.text())
            self.y_min_input.setText(Common.ymin_input_zy.text())
            self.y_max_input.setText(Common.ymax_input_zy.text())
