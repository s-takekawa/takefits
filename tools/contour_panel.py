...
from core.contour_manager import (
    ContourManager,
    ContourParameters,
...
from core.contour_manager import write_state_to_ds9, read_state_from_ds9
...
class ContourPanel(QDialog):
    ...
    def execute_contours(self) -> None:
        ...
        # Only update the global parameters if we are generating new contours.
        if layer_ids:
            self.manager.update_parameters(params)
            #self.manager.clear_layers(layer_ids)
            self.manager.apply_to_layers(layer_ids)

        if overlay_ids:
            self.manager.apply_overlays(overlay_ids, params.color, params.linewidth)
        self.refresh_targets()
    ...
    def clear_contours(self) -> None:
        ...
        if layer_ids:
            self.manager.clear_layers(layer_ids)
        if overlay_ids:
            self.manager.clear_overlays(overlay_ids)
        self.refresh_targets()
    ...
    def save_contours(self) -> None:
        ...
        if default_basename:
            default_filename = f"{default_basename}.ctr"
        else:
            ...
            default_filename = f"{base_component}.ctr"
        default_path = os.path.join(default_dir, default_filename)

        options = "DS9 Contour (*.ctr *.con *.txt);;Contour JSON (*.json);;All Files (*)"
        ...
        if not ext:
            if selected_filter.startswith("DS9"):
                path = f"{path}.ctr"
            elif selected_filter.startswith("Contour JSON"):
                path = f"{path}.json"
    ...
    def load_contours(self) -> None:
        ...
        try:
            if selected_filter.startswith("DS9") or os.path.splitext(path)[1].lower() in {".con", ".ctr", ".txt"}:
        ...
        for target_id in layer_ids:
            ...
            # Apply the UI's style settings to this new overlay.
            if target_state.parameters:
                target_state.parameters.color = style_color
                target_state.parameters.linewidth = style_linewidth

            # This ensures the new style is used, not any per-segment colors from the file.
