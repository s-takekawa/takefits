...
class UnitConversionPanel(QWidget):
    ...
    def apply_unit_conversion(self):
        ...
        try:
            ...
            method = 'Rayleigh-Jeans' if 'Rayleigh-Jeans' in self.method_combo.currentText() else 'Planck'

            scaled_data = None
            new_bunit_internal = self.current_bunit
            factor_values = None
            factor_direction = None

            current_unit = self.current_bunit

            if current_unit == 'jy/beam' and target_unit == 'k':
                if method == 'Rayleigh-Jeans':
                    scaled_data, factor_values = self._convert_jyb_to_k(method, return_factor=True)
                else:
                    scaled_data = self._convert_jyb_to_k(method)
                new_bunit_internal = 'k'
                factor_direction = 'I_nu -> Tb'

            elif current_unit == 'k' and target_unit == 'jy/beam':
                if method == 'Rayleigh-Jeans':
                    scaled_data, factor_values = self._convert_k_to_jyb(method, return_factor=True)
                else:
                    scaled_data = self._convert_k_to_jyb(method)
                new_bunit_internal = 'jy/beam'
                factor_direction = 'Tb -> I_nu'

            elif current_unit == 'jy/beam' and target_unit == 'jy/pix':
                scaled_data = self._convert_jyb_to_jypix()
                new_bunit_internal = 'jy/pix'

            elif current_unit == 'jy/pix' and target_unit == 'jy/beam':
                scaled_data = self._convert_jypix_to_jyb()
                new_bunit_internal = 'jy/beam'

            elif current_unit == 'k' and target_unit == 'jy/pix':
                if method == 'Rayleigh-Jeans':
                    intermediate_data, factor_values = self._convert_k_to_jyb(method, return_factor=True)
                else:
                    intermediate_data = self._convert_k_to_jyb(method)
                scaled_data = self._convert_jyb_to_jypix(data_in=intermediate_data)
                new_bunit_internal = 'jy/pix'
                factor_direction = 'Tb -> I_nu'

            elif current_unit == 'jy/pix' and target_unit == 'k':
                intermediate_data = self._convert_jypix_to_jyb()
                if method == 'Rayleigh-Jeans':
                    scaled_data, factor_values = self._convert_jyb_to_k(method, data_in=intermediate_data, return_factor=True)
                else:
                    scaled_data = self._convert_jyb_to_k(method, data_in=intermediate_data)
                new_bunit_internal = 'k'
                factor_direction = 'I_nu -> Tb'

            if scaled_data is not None:
                target_bunit_str = new_bunit_internal.upper().replace('JY', 'Jy').replace('PIX', 'pixel')
                history_msg = f"Unit conversion: {self.current_bunit.upper()} -> {target_bunit_str} (Method:{method})"

                if method == 'Rayleigh-Jeans' and factor_values is not None and factor_direction:
                    factor_summary = self._format_scaling_factor(factor_values)
                    history_msg += f"; RJ factor ({factor_direction}): {factor_summary}"

                new_bunit_header = new_bunit_internal.upper().replace('PIX','PIXEL') 
                self._apply_data_and_history(scaled_data, new_bunit_header, history_msg, history_category='conversion')

                self.current_bunit = new_bunit_internal 
    ...
    def _format_scaling_factor(self, factor_values):
        """Format Rayleigh-Jeans scaling factors for HISTORY logging."""
        if factor_values is None:
            return 'n/a'

        arr = np.asarray(factor_values, dtype=float)
        if arr.size == 1:
            return f"{float(arr.ravel()[0]):.6e}"

        finite = arr[np.isfinite(arr)]
        if finite.size == 0:
            return 'n/a'

        factor_min = float(finite.min())
        factor_max = float(finite.max())
        if np.isclose(factor_min, factor_max, rtol=1e-6, atol=0):
            return f"{factor_min:.6e}"
        return f"min={factor_min:.6e}, max={factor_max:.6e}"
    ...
    def reset_conversion_operations(self):
        ...
        data_was_scaled = self.original_data is not None

        if data_was_scaled:
            QMessageBox.information(self, 'Conversion Reset', 'Unit conversions have been reset.')
    ...
    def _update_history(self, operation_details: str, category: Optional[str] = None, include_general: bool = True):
        ...
        operation_entry = f"Operation details: {operation_details}"
        general_entry = (
        ...
        for header in headers_to_update:
            ...
            self._insert_history_entries(header, general_entry, operation_entry, category)

        self._history_dedup_keys.add(dedup_key)

        if category:
            entries_to_store = tuple(
                entry for entry in (general_entry, operation_entry) if entry
            )
            self._history_categories[category] = entries_to_store

    def _insert_history_entries(self, header, general_entry, operation_entry, category):
        ...
        if general_entry and general_entry not in entries:
            entries.append(general_entry)
            modified = True

        if operation_entry and operation_entry not in entries:
            ...
            if insert_index >= len(entries):
                ...
                entries.insert(insert_index, operation_entry)
            modified = True
    ...
    def _get_freq_axis_hz(self):
        """
        Get the frequency of each channel in Hz.
        Returns a 1D array or a scalar.
        """
        ...
        if 'RESTFRQ' in header:
            ...
            if n_channels <= 1: return restfreq_hz
        ...
        if spec_axis_num != -1:
            ...
            if n_channels <= 1: return crval * factor
    ...
    def _reshape_freqs_for_broadcast(self, freqs_hz, data):
        """Reshape frequency array for broadcasting with data array."""
        n_dims = data.ndim
    ...
    def _convert_jyb_to_k(self, method, data_in=None, return_factor=False):
        """Convert Jy/beam data to K (Brightness Temperature).

        When return_factor is True and method is Rayleigh-Jeans, returns
        a tuple of (scaled_data, factor) where factor may be an array.
        """
        data = data_in if data_in is not None else self.fits_viewer.data
        if self.beam_area_sr == 0:
            raise ValueError("Cannot convert to K: Beam area is zero or BMAJ/BMIN missing.")
            
        freqs_hz = self._get_freq_axis_hz()

        if method == 'Rayleigh-Jeans':
            freqs_to_use = self._reshape_freqs_for_broadcast(freqs_hz, data)
            s_nu_si = (1.0 * 1e-26) / self.beam_area_sr 
            factor = (s_nu_si * (const.c.value**2)) / (2 * const.k_B.value * (freqs_to_use**2))
            scaled = data * factor
            return (scaled, factor) if return_factor else scaled

        elif method == 'Planck':
            freqs_reshaped = self._reshape_freqs_for_broadcast(freqs_hz, data)
            if not isinstance(freqs_reshaped, np.ndarray):
                 raise ValueError("Planck conversion requires a 3D+ cube.")
            
            s_nu_per_sr_val = (data * 1e-26) / self.beam_area_sr
            
            with np.errstate(divide='ignore', invalid='ignore'):                
                h, k, c = const.h.value, const.k_B.value, const.c.value
                term_h_nu_k = (h * freqs_reshaped) / k
                term_2h_nu3_c2 = (2.0 * h * freqs_reshaped**3) / (c**2)
                log_argument = 1.0 + (term_2h_nu3_c2 / s_nu_per_sr_val)
                scaled_data = term_h_nu_k / np.log(log_argument)
            return (scaled_data, None) if return_factor else scaled_data
            
    def _convert_k_to_jyb(self, method, data_in=None, return_factor=False):
        """Convert K (Brightness Temperature) data to Jy/beam.

        When return_factor is True and method is Rayleigh-Jeans, returns
        a tuple of (scaled_data, factor).
        """
        data = data_in if data_in is not None else self.fits_viewer.data
        if self.beam_area_sr == 0:
            raise ValueError("Cannot convert from K: Beam area is zero or BMAJ/BMIN missing.")
            
        freqs_hz = self._get_freq_axis_hz()

        if method == 'Rayleigh-Jeans':
            freqs_to_use = self._reshape_freqs_for_broadcast(freqs_hz, data)
            s_nu_si_per_k = (2 * const.k_B.value * (freqs_to_use**2)) / (const.c.value**2)
            factor = (s_nu_si_per_k * self.beam_area_sr) / 1e-26
            scaled = data * factor
            return (scaled, factor) if return_factor else scaled
            ...
            with np.errstate(over='ignore', divide='ignore', invalid='ignore'):
                ...
                term_h_nu_k = (h * freqs_reshaped) / k
                term_2h_nu3_c2 = (2.0 * h * freqs_reshaped**3) / (c**2)
                exp_term = np.exp(term_h_nu_k / data)
                s_nu_per_sr = term_2h_nu3_c2 / (exp_term - 1.0)
                scaled_data = (s_nu_per_sr * self.beam_area_sr) / 1e-26
            return (scaled_data, None) if return_factor else scaled_data

    def _convert_jyb_to_jypix(self, data_in=None):
        """Convert Jy/beam data to Jy/pixel."""
        data = data_in if data_in is not None else self.fits_viewer.data
        if self.beam_area_sr == 0 or self.pixel_area_sr == 0:
             raise ValueError("Cannot convert Jy/beam to Jy/pix: Beam or Pixel area is zero.")
        return data * (self.pixel_area_sr / self.beam_area_sr)

    def _convert_jypix_to_jyb(self, data_in=None):
        """Convert Jy/pixel data to Jy/beam."""
        data = data_in if data_in is not None else self.fits_viewer.data
        if self.beam_area_sr == 0 or self.pixel_area_sr == 0:
             raise ValueError("Cannot convert Jy/pix to Jy/beam: Beam or Pixel area is zero.")
        return data * (self.beam_area_sr / self.pixel_area_sr)
