...
from core.region import CircleRegion, CubeRegion, EllipseRegion, RectangleRegion, Region
    ...
    def _handle_save(self) -> None:
        try:
            ...
            processor = CutoutProcessor(
                ...
                self.header,
                self.wcs,
                wcs_to_data_axis=self._wcs_to_data_axis,
            )
            cutout_data, new_header, _, actual_pixel_bounds = processor.extract(pixel_bounds)
            ...
            """
            try:
                debug_units = [save_header.get(f'CUNIT{idx + 1}', '') for idx in range(save_header.get('NAXIS', 0))]
                print('CutOut save header units:', debug_units)
            except Exception:
                pass
            """
