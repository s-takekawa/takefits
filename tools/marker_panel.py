...
class MarkerPanel(QDialog):
    ...
    def _list_entry_text(self, marker: Marker) -> str:
        ...
        color_value = marker.style.color or "auto"
    ...
    def _populate_detail_fields(self, marker: Marker, *, preserve_selection: bool = False) -> None:
        ...
        try:
            ...
            style = marker.style
            display = self._kind_display(marker)
            type_index = next(
    ...
    def _update_position_fields(self, marker: Marker) -> None:
        ...
        if world_editable:
            try:
                ...
                formatted = converter.pix_to_world(*pixel_vector)
                wx = formatted[axis_indices[0]]
                wy = formatted[axis_indices[1]]
                world_texts = (str(wx), str(wy))
            except Exception:
                world_texts = None

        if viewer is not None:
            format_pix = getattr(viewer, "format_pix", None)
            convert_fn = getattr(format_pix, "convert", None) if format_pix else None
            if callable(convert_fn):
                try:
                    converted = convert_fn(marker.plane, x_pix, y_pix)
                    if isinstance(converted, tuple) and len(converted) >= 2:
                        world_texts = (str(converted[0]), str(converted[1]))
                except Exception:
                    pass
    ...
    def _world_strings_from_marker(
        ...
        *,
        ...
        if viewer is not None:
            ...
            if callable(convert_fn):
                try:
                    converted = convert_fn(marker.plane, marker.pixel[0], marker.pixel[1])
                    if isinstance(converted, tuple) and len(converted) >= 2:
                        return (str(converted[0]), str(converted[1]))
                except Exception:
                    pass
        ...
        if converter is not None:
            config = getattr(converter, "config", None)
            ...
            if axis_types:
                ...
                for value, axis_idx in zip(world, axis_indices):
                    if axis_idx is not None and 0 <= axis_idx < len(axis_types):
                        try:
                            formatted = converter.format_world_coordinate(value, axis_types[axis_idx])
                        except Exception:
                            formatted = None
                        if formatted is not None:
                            formatted_values.append(str(formatted))
                            continue
                    formatted_values.append(f"{value:.{decimals}f}")
                if len(formatted_values) == 2:
                    return (formatted_values[0], formatted_values[1])
        return (f"{world[0]:.{decimals}f}", f"{world[1]:.{decimals}f}")
    ...
    def _plane_axis_indices(self, plane: Optional[str], wcs) -> Optional[Tuple[int, int]]:
        ...
        if plane_key == "xy" and naxis >= 2:
            return (0, 1)
        if plane_key == "xz" and naxis >= 3:
            return (0, 2)
        if plane_key == "zy" and naxis >= 3:
            return (2, 1)
        return None
    ...
    def _build_pixel_vector(self, plane: str, x_pix: float, y_pix: float, wcs) -> List[float]:
        naxis = getattr(wcs, "naxis", 0) or 0
        plane_key = self._base_plane(plane)
        x_fixed = float(getattr(Common, "xpix", 0.0))
        y_fixed = float(getattr(Common, "ypix", 0.0))
        z_fixed = float(getattr(Common, "zpix", 0.0))
        stokes_fixed = float(getattr(Common, "spix", 0.0))
        vector: List[float] = []
    ...
    def _apply_line_style(self) -> None:
        if self._suspend_updates:
            return
        markers = [m for m in self._selected_markers() if isinstance(m, LineMarker)]
        if not markers:
            marker = self._current_marker()
            if isinstance(marker, LineMarker):
                markers = [marker]
    ...
    def _apply_text_style(self) -> None:
        if self._suspend_updates:
            return
        markers = [m for m in self._selected_markers() if isinstance(m, TextMarker)]
        if not markers:
            marker = self._current_marker()
            if isinstance(marker, TextMarker):
                markers = [marker]
