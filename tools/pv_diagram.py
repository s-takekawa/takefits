...
from PyQt6.QtWidgets import QMainWindow, QWidget, QHBoxLayout, QVBoxLayout, QPushButton, QGroupBox, QFormLayout, QCheckBox, QComboBox, QDoubleSpinBox, QLabel, QButtonGroup, QRadioButton, QLineEdit, QGridLayout, QFileDialog, QMessageBox
import time
import math
import os
import re
...
class PVNavigationToolbar(MyNavigationToolbar):
    ...
    def save_figure(self):
        ...
        if path:
            ...
            try:
                # Save with high resolution and transparency
...
class PVdiagram(QMainWindow):
    def __init__(self, fits_viewer):
        ...
        try:
            initial_vel_min = Common.zmin_input_xz.text()
            initial_vel_max = Common.zmax_input_xz.text()
            ...
            print("Warning: Could not sync initial velocity range for PV diagram. Common attributes might not be ready.")
        except Exception as e:
            print(f"Error during initial velocity range sync: {e}")

        try:
            initial_channel = Common.slider_xy.value()
        ...
        try:
            initial_vel_min_str = Common.zmin_input_xz.text()
            initial_vel_max_str = Common.zmax_input_xz.text()
    ...
    def initUI(self):
        ...
        self.fits_ax = Common.overlay_ax_xy
        self.fits_ax.set_autoscale_on(False)
        self.fits_canvas = Common.canvas_xy
        ...
        self.sliceWidthSpin.setRange(1, 99)
        ...
        interpGroup = QButtonGroup(self)
        bilinearRadio = QRadioButton("Bilinear")
        gaussianRadio = QRadioButton("Gaussian")
        bilinearRadio.setChecked(True)
        interpGroup.addButton(bilinearRadio, 0)
        interpGroup.addButton(gaussianRadio, 1)
        interpGroup.buttonClicked.connect(lambda btn: setattr(self, "weight_mode", interpGroup.checkedId()))
        interp_layout = QHBoxLayout()
        interp_layout.addWidget(bilinearRadio)
        interp_layout.addWidget(gaussianRadio)
    ...
    def save_fits(self):
        """Saves the current PV diagram data as a new 2D FITS file with generated WCS."""
        if self.line_start is None or self.line_end is None:
        ...
        header['BUNIT'] = self.fits_viewer.header.get('BUNIT', 'UNKNOWN') # Inherit original Bunit

        # --- Define Position Axis (Offset in degrees) ---
        # Calculate position axis properties based on user requirement "unit = deg"
        line_length_px = np.hypot(self.line_end[0] - self.line_start[0],
                                  self.line_end[1] - self.line_start[1])
        length_deg = self._convert_length(line_length_px, 'pixel', 'deg')
        ...
        # Calculate CDELT for position axis (deg/pixel)
        if num_pos_pixels > 1:
        ...
        # --- Define Velocity Axis (Inherited from original cube) ---
        original_wcs = self.fits_viewer.wcs
        vel_axis_index = 2 # Assuming velocity is axis 3 (index 2) in the original cube WCS
        
        original_header = self.fits_viewer.header
        # Get CTYPE, CRVAL, CDELT, CRPIX from original WCS
        vel_ctype_key = f'CTYPE{vel_axis_index + 1}' # CTYPE3
        if vel_ctype_key in original_header:
            vel_ctype = original_header[vel_ctype_key]
        else:
            vel_ctype = 'VELOCITY'
        vel_crval = original_wcs.wcs.crval[vel_axis_index] if original_wcs.wcs.naxis >= 3 else 0.0
        vel_cdelt = original_wcs.wcs.cdelt[vel_axis_index] if original_wcs.wcs.naxis >= 3 else 1.0
        vel_crpix = original_wcs.wcs.crpix[vel_axis_index] if original_wcs.wcs.naxis >= 3 else 1.0

        # ★ Get CUNIT from the displayed axis label for accuracy (e.g., to ensure km/s) ★
        vel_label_str = Common.ax_coord_xz[1].get_axislabel() # Get "Velocity [km/s]" etc.
        match = re.search(r'\[(.*?)\]', vel_label_str)
        if match:
            vel_cunit = match.group(1).strip() # Extract unit from brackets, e.g., "km/s"
        else:
            # Fallback if label parsing fails
            vel_cunit = original_wcs.wcs.cunit[vel_axis_index].to_string() if original_wcs.wcs.naxis >= 3 else 'km/s'
            # Ensure "m/s" gets converted if velocity conversion was applied but label parsing failed somehow
            if vel_cunit.lower() == 'm/s' and np.abs(vel_cdelt) > 100: # Heuristic for m/s data displayed as km/s
                 vel_cunit = 'km/s'
        ...
        for key in ['SPECSYS', 'RESTFRQ', 'VELREF', 'TELESCOP', 'INSTRUME', 'OBJECT']:
            if key in original_header:
                header[key] = original_header[key]

        # --- 3. Add HISTORY (Recommended) ---
        header['HISTORY'] = f"PV Diagram created by takefits on {datetime.now().isoformat()}"
        header['HISTORY'] = f"Source FITS: {self.fits_viewer.filename}"
        header['HISTORY'] = f"Slice Start (pix): ({self.line_start[0]:.2f}, {self.line_start[1]:.2f})"
        header['HISTORY'] = f"Slice End (pix): ({self.line_end[0]:.2f}, {self.line_end[1]:.2f})"
        try:
            ...
            header['HISTORY'] = f"Slice Start (world): ({start_world[0]}, {start_world[1]})"
            header['HISTORY'] = f"Slice End (world): ({end_world[0]}, {end_world[1]})"
        except Exception:
            header['HISTORY'] = "Could not convert slice endpoints to world coordinates."
        header['HISTORY'] = f"Slice Width (pix): {self.sliceWidthSpin.value()}"
        interp_mode = "Gaussian" if self.weight_mode == 1 else "Bilinear"
        header['HISTORY'] = f"Interpolation Mode: {interp_mode}"

        # --- 4. Save File ---
        save_fits_handler = SaveFITS(data_to_save, header, self.fits_viewer.filename)
        save_fits_handler.save(suffix="pv")
    ...
    def update_pv_diagram(self, force_update=False):
        ...
        if line_length_px == 0:
            num_samples = 1
            pv = np.full((self.data.shape[0], num_samples), np.nan)
        else:
            ...
            n_vel = self.data.shape[0]
            pv = np.zeros((n_vel, num_samples))

            if self.weight_mode == 0: # Bilinear interpolation
                if self.slice_width > 1:
                    n_offsets = int(round(self.slice_width))
                    offsets = np.linspace(-self.slice_width / 2., self.slice_width / 2., n_offsets)
                    for v in range(n_vel):
                        prof_sum = np.zeros(num_samples)
                        count = np.zeros(num_samples)
                        for off in offsets:
                            off_x = -np.sin(theta) * off
                            off_y = np.cos(theta) * off
                            prof = map_coordinates(self.data[v], [ys + off_y, xs + off_x],
                                                    order=1, mode='constant', cval=np.nan)
                            valid = ~np.isnan(prof)
                            prof_sum[valid] += prof[valid]
                            count[valid] += 1
                        result = np.full(num_samples, np.nan)
                        valid_mask = count > 0
                        result[valid_mask] = prof_sum[valid_mask] / count[valid_mask]
                        pv[v, :] = result
                else:
                    for v in range(n_vel):
                        pv[v, :] = map_coordinates(self.data[v], [ys, xs],
                                                   order=1, mode='constant', cval=np.nan)
            elif self.weight_mode == 1: # Gaussian weighting
                if self.slice_width > 0:
                    sigma = self.slice_width / (2 * np.sqrt(2 * np.log(2)))
                    n_offsets = int(np.ceil(self.slice_width * 2)) + 1
                    offsets = np.linspace(-self.slice_width / 2, self.slice_width / 2, n_offsets)
        ...
        n_vel = self.data.shape[0]
        vel_label = Common.ax_coord_xz[1].get_axislabel()
