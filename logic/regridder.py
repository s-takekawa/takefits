...
from astropy.coordinates import FK5, FK4, Galactic, ICRS, FK4NoETerms
...
class Regridder(QObject):
    """Backend worker that performs FITS regridding operations."""

    progress = pyqtSignal(int)
    finished = pyqtSignal(object, object)
    error = pyqtSignal(str)

    def __init__(self, original_data, original_wcs, original_header: Optional[fits.Header] = None, filename: Optional[str] = None):
        super().__init__()
    ...
    def _regrid_from_template_fits(self, params: Dict, method: str) -> Tuple[np.ndarray, fits.Header, WCS]:
        ...
        def _process_spatial_plane(plane_idx):
            slicer = [slice(None)] * self.original_data.ndim
            slicer[np_spectral_axis] = plane_idx
            plane_data = self.original_data[tuple(slicer)]
            if not isinstance(plane_data, np.ndarray):
                plane_data = np.asarray(plane_data)
            if plane_data.dtype != work_dtype:
                plane_data = plane_data.astype(work_dtype, copy=False)
            output_plane = np.empty(spatial_shape_out, dtype=work_dtype)
            reprojected_plane, _, used_fix, downgraded = _reproject_with_nan_support(
                plane_data,
                source_wcs_2d,
                target_wcs_2d,
            ...
            if downgraded:
                self._nan_high_order_downgraded = True
            return reprojected_plane

        cpu_count = os.cpu_count() or 1
        max_workers = min(8, max(1, cpu_count))
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(_process_spatial_plane, i): i for i in range(n_spec_orig)}
            completed_count = 0
            for future in as_completed(futures):
                idx = futures[future]
                slicer = [slice(None)] * data_spatial_regridded.ndim
                slicer[np_spectral_axis] = idx
                data_spatial_regridded[tuple(slicer)] = future.result()
                completed_count += 1
                self.progress.emit(20 + int(40 * (completed_count / n_spec_orig)))
        ...
        if not is_3d_cube:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                for fut in as_completed(futures):
                    ...
                    self.progress.emit(int(progress_start + (progress_end - progress_start) * ratio))
    ...
    def _annotate_history(
        ...
        *,
        ...
        if source_file:
            header.add_history(f"Source file: {source_file}")
        ...
        if mode == "manual":
            ...
            template_path = params.get("template_path", "")
            header.add_history(
                f"Template applied: {os.path.basename(template_path) if template_path else 'N/A'}"
            )
        elif mode == "reproject_system":
    ...
    def _normalize_beam_position_angle(angle: float, prefer_nonnegative: bool) -> float:
        ...
        if prefer_nonnegative:
            ...
            if angle < 0:
                angle += 180.0
            return angle
        return Regridder._wrap_angle_degrees(angle)
    ...
    def _regrid_manual(self, params: Dict, method: str) -> Tuple[np.ndarray, fits.Header, WCS]:
        ...
        if len(shape_out) < 3:
            ...
            def _process_plane(z_idx: int):
                # This logic correctly builds the coordinate array for wcs_pix2world
                ...
                # spatial_grids[0] are Y indices (numpy rows), spatial_grids[1] are X indices (numpy columns)
                # WCS typically expects (X, Y) order.
                coords_for_wcs_axes[spatial_wcs_indices[0]] = spatial_grids[1]  # Assign X grid to first spatial WCS axis
                coords_for_wcs_axes[spatial_wcs_indices[1]] = spatial_grids[0]  # Assign Y grid to second spatial WCS axis
                
                # The list `coords_for_wcs_axes` is now ordered according to WCS axes [X, Y, Z]
                # vstack and transpose will create the (N, 3) array with columns (X, Y, Z)
                ...
                # Use filled_data for interpolation
                # (filled_data and nan_mask are read from the outer scope)
