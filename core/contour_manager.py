    ...
    def _draw_overlay_state(self, state: ContourState) -> None:
        ...
        for idx, item_state in enumerate(state.items):
            ...
            for segment in item_state.segments:
                ...
                segment.pixels = coords
                if wcs is not None:
                    new_world = _pixel_coords_to_world(coords, wcs)
                    if new_world is not None:
                        segment.world = new_world
    ...
    def update_overlay_style(self, color: str, linewidth: float, overlay_ids: Optional[Iterable[str]] = None) -> bool:
        ...
        for state in self._overlay_states:
            if target_ids is not None and state.overlay_id not in target_ids:
                continue
            if state.parameters.color != color or state.parameters.linewidth != linewidth:
    ...
    def update(self, params: ContourParameters) -> Optional[ContourState]:        
        ...
        if self._owner is not None:
            ...
            if hasattr(owner, 'plane'):
                ...
                if plane == 'xy':
                    if Common.xy_vline: hidden_artists.append(Common.xy_vline)
                    if Common.xy_hline: hidden_artists.append(Common.xy_hline)
                    if Common.chlabel_xy: hidden_artists.append(Common.chlabel_xy)
                    if Common.hpbw_xy and Common.hpbw_xy.ellipse: hidden_artists.append(Common.hpbw_xy.ellipse)
                    ...
                    if Common.xz_vline: hidden_artists.append(Common.xz_vline)
                    if Common.xz_hline: hidden_artists.append(Common.xz_hline)
                    if Common.chlabel_xz: hidden_artists.append(Common.chlabel_xz)
                
                elif plane == 'zy':
                    if Common.zy_vline: hidden_artists.append(Common.zy_vline)
                    if Common.zy_hline: hidden_artists.append(Common.zy_hline)
                    if Common.chlabel_zy: hidden_artists.append(Common.chlabel_zy)
            ...
            for item in items:
                ...
                try:
                    ...
                    if use_cmap:
                        ...
                        contour_kwargs["colors"] = params.color
