...
class RegionEditorDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.world_frame_combo = QComboBox()
        ...
        form.addRow("World Frame", self.world_frame_combo)
    ...
    def _populate_world_frame_combo(self):
        getter = getattr(self.viewer, "_get_shared_display_frame", None)
        current = normalize_display_frame(getter() if callable(getter) else "native")
    ...
    def _current_world_frame(self) -> str:
        frame = normalize_display_frame(self.world_frame_combo.currentData())
    ...
    def _shared_world_vector(self):
        ...
        for name in ("_get_shared_world_x", "_get_shared_world_y", "_get_shared_world_z", "_get_shared_world_s"):
            getter = getattr(self.viewer, name, None)
            if callable(getter):
                try:
    ...
    def _build_pixel_vector(self, x_pix, y_pix, slices, wcs):
        ...
        for idx, entry in enumerate(slices):
            ...
            if any(key in axis_type_upper for key in ('VELO', 'VRAD', 'VOPT', 'FREQ')):
                default_value = float(self.viewer._get_shared_zpix())
            elif axis_type_upper.startswith('RA') or 'LON' in axis_type_upper:
                default_value = float(self.viewer._get_shared_xpix())
            elif 'DEC' in axis_type_upper or 'LAT' in axis_type_upper:
                default_value = float(self.viewer._get_shared_ypix())
    ...
    def _update_world_fields(self):
        ...
        try:
            ...
            world_x_val, world_y_val, axis_x, axis_y = plane_values_for_display(
                ...
                native_world[x_axis_idx_in_fits],
    ...
    def _on_world_edit_finished(self):
        ...
        world_x_str = self.centerWorldXEdit.text().strip()
        world_y_str = self.centerWorldYEdit.text().strip()
        if not world_x_str or not world_y_str:
            self._update_world_fields()
            return
        ...
        x_axis_idx_in_fits = slices.index('x')
        y_axis_idx_in_fits = slices.index('y')
        native_pair = plane_inputs_to_native(
            wcs,
            getattr(self.viewer, "plane", "xy"),
            world_x_str,
            world_y_str,
    ...
    def _on_world_frame_changed(self, _index):
        if self._updating_fields:
            return
        self._update_world_fields()
