...
def _require_sklearn():
    if silhouette_score is None or spectral_embedding is None or k_means is None:
        detail = f": {_SKLEARN_IMPORT_ERROR}" if _SKLEARN_IMPORT_ERROR is not None else ""
        raise ImportError(f"scikit-learn is required for SCIMES spectral clustering{detail}")

def _candidate_cluster_counts(guessed_k, n_samples):
    """Return valid cluster counts for silhouette search."""
    min_ks = max(2, guessed_k - 15)
    max_ks = min(guessed_k + 15, n_samples - 1)
    if max_ks < min_ks:
        return []
    return list(range(min_ks, max_ks + 1))


def _add_topology_columns(catalog, parents, ancestors, nleaves, struct_types):
    """Annotate a SCIMES catalog with dendrogram topology columns."""
    catalog['parent'] = np.asarray(parents)
    catalog['ancestor'] = ancestors
    catalog['n_leaves'] = nleaves
    catalog['structure_type'] = np.asarray(struct_types)
...
def mat_smooth(Mat, S2Nmat, s2nlim = 3, scalpar = None, lscal = False):
    ...
    if scalpar is not None:
        try:
            scalpar_value = scalpar.to_value()
        except AttributeError:
            scalpar_value = scalpar
        print("-- User defined scaling parameter: %f" % scalpar_value)
        sigma = scalpar_value**2
...
def aff_matrix(num, trk, allleavidx, allbrcidx, brclevels, dictchildrens, props):
    ...
    for l in np.unique(brclevels):
        ...
        for p in lbrcidx:
            ...
            pleavpos = allleavpos[np.isin(allleavidx,pleavidx)]
...
def cloudstering(dendrogram, catalog, criteria, user_k, user_ams, user_scalpars, user_iter,
    save_isol_leaves, save_clust_leaves, save_branches, blind, rms, s2nlim, locscal, criteria_weights=None):
    ...
    if AMs.shape[1] <= 2:
        ...
        _add_topology_columns(catalog, all_struct_parents, all_struct_ancestors,
                              nleaves, all_struct_types)

        return clusts, catalog, AMs, [], 0.0
    ...
    # Criteria weights: list aligned to `criteria` or dict keyed by criterion name.
    weights = []
    if isinstance(criteria_weights, dict):
        for crit in criteria:
            try:
                weights.append(float(criteria_weights.get(crit, 1.0)))
            except Exception:
                weights.append(1.0)
    elif criteria_weights is None:
        weights = [1.0] * len(criteria)
    else:
        try:
            weights = [float(w) for w in criteria_weights]
        except Exception:
            weights = [1.0] * len(criteria)
        if len(weights) < len(criteria):
            weights = weights + [1.0] * (len(criteria) - len(weights))
    ...
    if kg > 1:
        ...
        sils = []
        valid_ks = []
        candidate_ks = _candidate_cluster_counts(kg, rAM.shape[0])
        clust_configs = []

        if len(candidate_ks) == 0:
            print("-- Not enough reduced leaves for silhouette search")
            all_clusters = np.zeros(len(mul_leav_idx), dtype = np.int32)
            ...
            for ks in candidate_ks:
                ...
                try:
                    ...
                    _, curr_clusters, _ = k_means(evecs, ks, random_state=222, n_init=user_iter)

                    sil = silhouette_score(evecs, np.asarray(curr_clusters), metric='euclidean')

                    clust_configs.append(curr_clusters)
                    sils.append(sil)
                    valid_ks.append(ks)
            ...
            if len(clust_configs) == 0:
                print("-- Could not evaluate silhouette candidates")
                all_clusters = np.zeros(len(mul_leav_idx), dtype = np.int32)
                ...
                best_ks = valid_ks[best_idx]
                silhoutte = sils[best_idx]
                print("-- Best cluster number found through SILHOUETTE (%f)= %i" % (silhoutte, best_ks+len(two_clust_idx)))

                all_clusters = clust_configs[best_idx]
                selected_ks = best_ks
    ...
    for clust in clusts:
        ...
        pos = np.where(np.isin(mul_leav_idx,fleavs_idx))[0]
        fclusts_idx[pos] = i
    ...
    unique_final = np.unique(fclusts_idx)
    if selected_ks is not None and 1 < len(unique_final) < len(fclusts_idx):
        try:
            evecs = spectral_embedding(rAM, n_components=selected_ks,
                                    eigen_solver='arpack',
                                    random_state=222,
                                    eigen_tol=0.0, drop_first=False)
            ...
            print("-- Final clustering configuration silhoutte unavailable")
    else:
        print("-- Final clustering configuration silhoutte unavailable")
...
class SpectralCloudstering(object):
    ...
    def __init__(self, dendrogram, catalog, header, criteria = ['volume', 'luminosity'], criteria_weights=None,
        ...
        if user_k is None:
            ...
            try:
                self.user_k = int(getattr(user_k, "value", user_k))
            except Exception:
                self.user_k = 0
        self.user_ams = user_ams
        if user_scalpars is None:
            self.user_scalpars = [None]*len(criteria)
        else:
        ...
        if user_iter is None:
            ...
            try:
                self.user_iter = int(getattr(user_iter, "value", user_iter))
            except Exception:
                self.user_iter = 1
        ...
        # ... of clusters (this is used by TakeFITS)
        # self.clusters_asgn = make_asgncube(self.dendrogram, self.clusters, self.header, collapse = False)
        self.clusters_asgn = None
        # ... of trunks (SKIPPED for performance - not used by TakeFITS)
        ...
        self.trunks_asgn = None
        # ... of leaves (SKIPPED for performance - not used by TakeFITS)
