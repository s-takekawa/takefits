...
from takefits.core.usecases import compute_polynomial_baseline_subtraction, get_averaged_spectrum
...
class BaselinePanel(BaseToolPanel):
    ...
    def save_subtracted_fits(self):
        ...
        header.add_history(f"Baseline-subtracted cube generated by takefits on {timestamp}")
        header.add_history(f"Source file: {self.fits_viewer.filename}")
        header.add_history(f"Polynomial order: {int(self.baseline_order_spinbox.value())}")
        try:
            ranges = self._collect_baseline_world_ranges(strict=True)
            if ranges:
                range_text = ", ".join(f"[{lo:.6g}, {hi:.6g}]" for lo, hi in ranges)
                header.add_history(f"Baseline world ranges: {range_text}")
        except Exception:
            pass
    ...
    def save_baseline_model_fits(self):
        ...
        header = self.fits_viewer.header.copy()
        finite = np.isfinite(self.baseline_model_data)
        if np.any(finite):
            header["DATAMIN"] = float(np.nanmin(self.baseline_model_data))
            header["DATAMAX"] = float(np.nanmax(self.baseline_model_data))
        else:
            header.pop("DATAMIN", None)
            header.pop("DATAMAX", None)

        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        header.add_history(f"Baseline model generated by takefits on {timestamp}")
        header.add_history(f"Source file: {self.fits_viewer.filename}")
        header.add_history(f"Polynomial order: {int(self.baseline_order_spinbox.value())}")
        try:
            ranges = self._collect_baseline_world_ranges(strict=True)
        ...
        saver = SaveFITS(self.baseline_model_data, header, self.fits_viewer.filename)
        saver.save(suffix="baseline_model")
